...
interface EditChargeDialogProps {
  ...
  onUpdateCharge: (charge: ChargeType) => void;
}
...
export function EditChargeDialog({
  ...
  // Reset form when charge changes
  useEffect(() => {
    if (charge && open) {
      ...
      if (charge.hasSubCharges) {
        ...
        reset({
          ...
          status: charge.status,
          code: charge.code || "",
          entityType: charge.entityType || "ALL",
        });
      }
    }
  }, [charge, open, reset]);
  ...
  const onSubmit = (data: EditChargeInput) => {
    if (!charge) return;

    const updatedCharge: ChargeType = {
      ...charge,
      name: data.name,
    ...
    if (data.hasSubCharges) {
      ...
      // Entity type is determined by charge category
      updatedCharge.entityType = charge.chargeCategory as ChargeEntityType;
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Modify the charge type details and settings
          </DialogDescription>
        </DialogHeader>
